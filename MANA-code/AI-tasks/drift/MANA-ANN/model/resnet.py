...
class ResNet(nn.Module): # ANN version
    def __init__(self, block, layers, zero_init_residual=False,
        ...
        self.fc_layers = nn.Sequential(
            nn.Linear(11200, 512), # 11200 if MNIST, 90112 if DVS
            nn.ReLU(),
...
def _resnet(arch, block, layers, pretrained):
    model = ResNet(block, layers)  # Bottleneck, [3, 4, 23, 3]
    return model


def get_resnet(name, momentumn, pretrained=False):
    global bn_momentum
