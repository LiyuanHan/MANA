...
from Align_KD3A.spikingjelly.activation_based import neuron, encoding, functional, surrogate, layer
    ...
    def __init__(self, num_input_channels, num_init_features=16, small_input=True):
        ...
        if small_input:
            self.add_module('conv0',
                            layer.Conv2d(num_input_channels, num_init_features, kernel_size=3, stride=1, padding=1,
                                      bias=True))
        else:
            self.add_module('conv0',
                            layer.Conv2d(num_input_channels, num_init_features, kernel_size=7, stride=2, padding=3,
                                      bias=True))
            self.add_module('pool0', layer.MaxPool2d(kernel_size=3, stride=2, padding=1,
                                                  ceil_mode=False))
...
class _WideResUnit(nn.Module):
    def __init__(self, num_input_features, num_output_features, stride=1, drop_rate=0.3):
        ...
        self.f_block.add_module('norm1', layer.BatchNorm2d(num_input_features))
        self.f_block.add_module('IF1', neuron.IFNode(surrogate_function=surrogate.ATan()))
        self.f_block.add_module('conv1', layer.Conv2d(num_input_features, num_output_features,
                                                   kernel_size=3, stride=stride, padding=1, bias=False))
        self.f_block.add_module('dropout', layer.Dropout(drop_rate))
        self.f_block.add_module('norm2', layer.BatchNorm2d(num_output_features))
        self.f_block.add_module('IF2', neuron.IFNode(surrogate_function=surrogate.ATan()))
        self.f_block.add_module('conv2', layer.Conv2d(num_output_features, num_output_features,
                                                   kernel_size=3, stride=1, padding=1, bias=False))

        if num_input_features != num_output_features or stride != 1:
            self.i_block = nn.Sequential()
            self.i_block.add_module('norm', layer.BatchNorm2d(num_input_features))
            self.i_block.add_module('IF', neuron.IFNode(surrogate_function=surrogate.ATan()))
            self.i_block.add_module('conv',
                                    layer.Conv2d(num_input_features, num_output_features, kernel_size=1, stride=stride,
                                              bias=False))
...
class WideResNet(nn.Module):
    def __init__(self, num_input_channels=3, num_init_features=16, depth=28, width=2,
                 data_parallel=True, small_input=True, drop_rate=0.0, T=50):
        ...
        trans = nn.Sequential()
        trans.add_module("norm", layer.BatchNorm2d(widths[-1]))
        trans.add_module("IF", neuron.IFNode(surrogate_function=surrogate.ATan()))
        if data_parallel:
    ...
    def forward(self, input_img):
        ...
        x_seq = input_img.unsqueeze(0).repeat(self.T, 1, 1, 1, 1)
        features_seq = self.encoder(x_seq)
        fr = features_seq.mean(0)
        return fr
