...
class BasicBlock(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None, groups=1,
        ...
        self.IFneuron = neuron.IFNode(surrogate_function=surrogate.ATan())
...
class ResNet(nn.Module):
    def __init__(self, T, block, layers, zero_init_residual=False,
        ...
        self.fc_layers = nn.Sequential(
            layer.Linear(90112, 512),
            neuron.LIFNode(surrogate_function=surrogate.ATan()),
            layer.BatchNorm1d(num_features=512),
            layer.Dropout(p=0.75),
        )
    ...
    def _forward_impl(self, x):
        ...
        x_seq = self.fc_layers[2:](x_seq.view(self.T, x.size(0), -1, 1))
