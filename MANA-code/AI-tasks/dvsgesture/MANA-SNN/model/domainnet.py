...
feature_dict = {"resnet18": 512, "resnet34": 512, "resnet50": 2048, "resnet101": 512}

class DomainNet_encoder(nn.Module):
    def __init__(self, time_window, backbone, bn_momentum, pretrained=True, data_parallel=True):
        super(DomainNet_encoder, self).__init__()
        self.linear_outputs = None
        encoder = get_resnet(time_window, backbone, momentumn=bn_momentum, pretrained=pretrained)
        """if data_parallel:
...
#         super(DomainNetClassifier, self).__init__()
#         linear = nn.Sequential()
#         linear.add_module("fc", nn.Linear(1024, classes))
#         """if data_parallel:
#             self.linear = nn.DataParallel(linear)
#         else:
#             self.linear = linear"""
