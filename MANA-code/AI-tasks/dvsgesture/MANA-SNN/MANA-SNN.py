...
parser.add_argument('--time_window', default=10, type=int, help='SNN simulation time window length')
...
seed_set = np.array([31, 46, 49, 54, 55])
...
def main(args=args, configs=configs):
	...
	print("SNN using total time steps =", time_window)
		...
		for turn in range(turns):
			...
			if configs["DataConfig"]["dataset"] == "DomainNet":
				...
				neural_train, neural_test, label_train, label_test = split_data(
					all_micro_spikes_concat, 		# torch.Size([28000, 28]) torch.Size([28000, 28]) torch.Size([28000, 1]) torch.Size([28000, 1])
