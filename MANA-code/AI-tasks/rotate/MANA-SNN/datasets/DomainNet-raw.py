...
def get_domainnet_dloader_new(base_path, domain_name, batch_size, num_workers):
    # dataset_path = base_path  # path.join(base_path, 'dataset', 'DomainNet')
    
    # train_data_paths, train_data_labels = read_domainnet_data(dataset_path, domain_name, split="train")
    # test_data_paths, test_data_labels = read_domainnet_data(dataset_path, domain_name, split="test")
    
    x_train, y_train = load_train_data_8_class_multi_domain_train(domain_name)
    x_test, y_test = load_test_data_8_class_multi_domain_test(domain_name)
    
    Transform = transforms.Compose([transforms.ToTensor(), transforms.Normalize((0.5,), (0.5,))])

    # train_dataset = DomainNet(x_train, y_train, transforms_train, domain_name)
    # test_dataset = DomainNet(test_data_paths, test_data_labels, transforms_test, domain_name)
...
def get_domainnet_dloader_target(base_path, domain_name, batch_size, num_workers):
    x_test_data, y_test_data, x_train_data, y_train_data = load_target(domain_name)
    Transform = transforms.Compose([transforms.ToTensor(), transforms.Normalize((0.5,), (0.5,))])
    target_dataset_test = CustomDataset(x_test_data, y_test_data, transform=Transform)
    target_dataset_train = CustomDataset(x_train_data, y_train_data, transform=Transform)
    target_dloader_test = DataLoader(target_dataset_test, batch_size=batch_size, num_workers=num_workers, pin_memory=True, shuffle=True)
    target_dloader_train = DataLoader(target_dataset_train, batch_size=batch_size, num_workers=num_workers, pin_memory=True, shuffle=True)
    return target_dloader_train, target_dloader_test


def load_target(domain_name):
    target_path = '/home/wangqingyu/IL/PMNIST/'
    Flag = 0
    for i in range(600):  # 600 domain
        if i in domain_name:
            # import pdb; pdb.set_trace()
            file_data = target_path + str(i) + '.npz'
            target_data = np.load(file_data, allow_pickle=True)
            x_target = target_data['data'][()]['x']
            y_target = target_data['data'][()]['y']
            """x_target_train = np.expand_dims(x_target[:10], axis=0)
            y_target_train = np.expand_dims(y_target[:10], axis=0)"""
            x_target_train = x_target[:10]
            y_target_train = y_target[:10]
            x_target_test = x_target[11:]
            y_target_test = y_target[11:]
            del target_data
            if Flag == 0:
                x_test_data = x_target_test
                y_test_data = y_target_test
                x_train_data = x_target_train
                y_train_data = y_target_train
                Flag = 1
            else:
                x_test_data = np.concatenate((x_test_data, x_target_test), axis=0)
                y_test_data = np.concatenate((y_test_data, y_target_test), axis=0)
                x_train_data = np.concatenate((x_train_data, x_target_train), axis=0)
                y_train_data = np.concatenate((y_train_data, y_target_train), axis=0)
        else:
            continue
    return x_test_data, y_test_data, x_train_data, y_train_data


def get_domainnet_dloader_source(base_path, domain_name, batch_size, num_workers):
    x_source, y_source = load_source(domain_name)
    Transform = transforms.Compose([transforms.ToTensor(), transforms.Normalize((0.5,), (0.5,))])
    source_dataset = CustomDataset(x_source, y_source, transform=Transform)
    source_dloader = DataLoader(source_dataset, batch_size=batch_size, num_workers=num_workers, pin_memory=True, shuffle=True)
    return source_dloader


def load_source(domain_name):
    source_path = '/home/wangqingyu/IL/PMNIST/'
    file_data = source_path + str(domain_name) + '.npz'
    source_data = np.load(file_data, allow_pickle=True)
    x_source = source_data['data'][()]['x']  # (36096, 3, 64, 64)
    y_source = source_data['data'][()]['y']
    del source_data
    return x_source, y_source


"""def test_file():
    target_path = '/home/wangqingyu/IL/permute/PMNIST/'
    file_data = target_path + str(22) + '.npz'
    target_data = np.load(file_data, allow_pickle=True)"""
