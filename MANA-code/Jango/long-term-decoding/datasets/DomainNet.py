...
def get_domainnet_dloader_train(base_path, domain_data, domain_label, domain_pos, batch_size, num_workers, length=4):
    ...
    test_pos = np.array(domain_pos)[indices]

    domain_data = np.delete(domain_data, indices, axis=0)
    domain_label = np.delete(domain_label, indices)
    domain_pos = np.delete(domain_pos, indices, axis=0)
    
    train_dataset = CustomDataset(domain_data, domain_label, domain_pos, transform=Transform)
...
def get_domainnet_dloader_test(base_path, domain_data, domain_label, domain_pos, batch_size, num_workers, mix_length, output_data=False):
    ...
    train_pos = np.array(domain_pos)[indices]
    ...
    if output_data:
        return (train_data, train_label, train_pos), (domain_data, domain_label, domain_pos)

    train_dataset = CustomDataset(train_data, train_label, train_pos, transform=Transform)
    test_dataset = CustomDataset(domain_data, domain_label, domain_pos, transform=Transform)

    train_dloader = DataLoader(train_dataset, batch_size=batch_size, num_workers=num_workers, pin_memory=True, shuffle=True)
    test_dloader = DataLoader(test_dataset, batch_size=1, num_workers=num_workers, pin_memory=True, shuffle=False)

    return  train_dloader, test_dloader

class CustomDataset(Dataset):
    def __init__(self, data, labels, pos=None, transform=None):
        self.data = data
        self.labels = labels
        self.pos = pos
