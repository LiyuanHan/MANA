...
class ResNet(nn.Module):
    def __init__(self, T, block, layers, zero_init_residual=False, groups=1, width_per_group=64, replace_stride_with_dilation=None, norm_layer=None, tau=2):
...
def _resnet(time_window, arch, block, layers, pretrained):
    if time_window == None:
        model = ResNet(10, block, layers)  # Bottleneck, [3, 4, 23, 3]
    else:
