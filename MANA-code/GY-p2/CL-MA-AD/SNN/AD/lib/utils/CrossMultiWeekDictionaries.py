...
def generate_domain_dict_and_day_from_to_dict(cross_week_num): # 根据跨周数，生成域长度字典和起止字典
    ...
    for day in range(first_days_of_week[4 + cross_week_num], 120):
...
def verify_day_in_test_range(day, cross_week_num):
    if day not in valid_range_for_cross_week_num(cross_week_num):
        valid_day_list = list(valid_range_for_cross_week_num(cross_week_num))
