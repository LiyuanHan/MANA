...
# import config files
with open(r"GY-p2/AD-1-model/config/{}".format(args.config)) as file:
...
for path in [path__dimReduced, path__aligned, path__aligned_unaligned, path__results, path__save_domain_weight, path__save_loss, path__save_w_hs]:
...
def main(args=args, configs=configs):
    ...
    domain_dic, day_from_to_dic = generate_domain_dict_and_day_from_to_dict(cross_week_num, train_weeks, method="domains", domain_num=1)
    ...
    for test_count in range(len(test_Day_list)):
        ...
        for turn in range(len(seed_set)):
            ...
            if configs["DataConfig"]["dataset"] == "DomainNet":
                ...
                with open(path__aligned_unaligned + f'/turn_{turn}/unaligned_GY-train_{Day_from}_to_{Day_to}-test_{variable_num}.pkl', 'wb') as f:
                ...
                with open(path__aligned_unaligned + f'/turn_{turn}/aligned_GY-train_{Day_from}_to_{Day_to}-test_{variable_num}-turn_{turn}.pkl', 'wb') as f:
