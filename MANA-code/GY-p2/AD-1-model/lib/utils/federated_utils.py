...
def create_domain_weight_(source_domain_num):
...
def federated_average(model_list, coefficient_matrix, batchnorm_mmd=True):
    ...
    if batchnorm_mmd:
        ...
        for key_data_pair_list in zip(*dict_item_list):
            source_data_list = [pair[1] * coefficient_matrix[idx] for idx, pair in enumerate(key_data_pair_list)]
        ...
        for parameter_list in zip(*named_parameter_list):
            source_parameters = [parameter[1].data.clone() * coefficient_matrix[idx] for idx, parameter in enumerate(parameter_list)]
...
def knowledge_vote(knowledge_list, confidence_gate, num_classes):
    ...
    # print(f'max_conf={max_conf}, confidence_gate={confidence_gate}')
    ...
    consensus_knowledge = torch.zeros(consensus_knowledge.size(0), num_classes).cuda().scatter_(1,consensus_knowledge.view(-1, 1), 1)
...
def decentralized_training_strategy_(communication_rounds, epoch_samples, batch_size, total_epochs):
    """
