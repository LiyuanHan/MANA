...
path_subpath__modelName = f'/MANA-SNN'
...
def main(args=args, configs=configs):
    ...
    def align_embedings_cross_days(cebra_pos_train, cebra_pos_test):
        ...
        if align_method == 1:
            ...
            Q = torch.linalg.pinv(cebra_pos_test.T) @ cebra_pos_test.T @ cebra_pos_train @ torch.linalg.pinv(
                cebra_pos_test)
    ...
    for test_count in range(len(test_Day_list)):
        ...
        for turn in range(5):
            ...
            if configs["DataConfig"]["dataset"] == "DomainNet":
                ...
                # # split data
                # neural_train, neural_test, label_train, label_test, random_indices = split_data(all_micro_spikes_concat, all_macro_conditions_concat, len_for_each_session_trial, Day_from, Day_to)  # direction
                ...
                if weather_together:
                    ...
                    # with open(filepath + '/cl_dir_train_stard_embeddings_' + week_name_from_to + '_gy-p2.pkl',
                    #           'rb') as f:
                    ...
                    len_stand = int(len(label_train) * 0.35 // expect_num_bins) * expect_num_bins # 取训练的部分作为标准
                ...
                def split_data_for_day(data_spike_train, data_label):
                    ...
                    split_day_label = []
                    # Save embeddings in current folder
                    len_stand = int(len(label_train) * 0.25 // expect_num_bins) * expect_num_bins  # 取训练的部分作为标准
