...
def get_domainnet_dloader_test__choose_mixlen_from_start(base_path, domain_data,domain_label, batch_size, num_workers, mix_length):

    Transform = transforms.Compose([transforms.ToTensor(), transforms.Normalize((0.5,), (0.5,))])
    # length = 30
    length = mix_length # for single-day testing
    indices = np.array([i for i in range(mix_length)])
    print(" ")
    print("The mixing data from target domain is being chosen sequentially.")
    print(" ")
    train_data = np.array(domain_data)[indices]
    train_label = np.array(domain_label)[indices]
    domain_data = np.delete(domain_data, indices, axis=0)
